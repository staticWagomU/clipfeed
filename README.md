...
resolveDate ────── picks pubDate per the filename / frontmatter / mtime strategy
      │
 buildFeed ─────── sort desc, deterministic tie-break, dedupe by normalized link, take top N
      │
 ┌────┴────┐
 ▼         ▼         ▼
...
141 tests cover every pure function plus the CLI orchestration layer (parse → build → render → write
